/**********************************************************
 * qico main
 * $Id: main.c,v 1.1.1.1 2003/07/12 21:27:03 sisoft Exp $
 **********************************************************/
#include "headers.h"
#include <stdarg.h>
#include <locale.h>
...
subst_t *psubsts;
int qipcr_msg,unhld,force=IPC_EXCL;
...
void daemon_mode()
{
	...
	while(1) {
		...
		if(t_rescan>=rescanperiod || do_rescan) {
			...
			do_rescan=0;
			if(!q_rescan(&current,!rnum--))write_log("can't rescan outbound %s!",cfgs(CFG_OUTBOUND));
			t_rescan=0;
		}
			...
			while(!dable&&i) {
				...
				if(checktimegaps(cfgs(CFG_CANCALL))&&find_dialable_subst(rnode,havestatus(f,CFG_IMMONFLAVORS),psubsts)) {
					...
					if(chld==0) {
						...
						if(!log_init(cfgs(CFG_MASTERLOG),NULL)) {
							fprintf(stderr,"can't init log %s.%s!",ccs,port);
						}
