 ...
 * $Id: slists.c,v 1.1 2004/06/24 09:53:32 sisoft Exp $
 **********************************************************/
#include "headers.h"
...
aslist_t *aslist_add(aslist_t **l,char *s,char *a)
{
	aslist_t **t;
	for(t=l;*t;t=&((*t)->next));
...
aslist_t *aslist_find(aslist_t *l,char *s)
{
	for(;l;l=l->next)if(!strcmp(l->str,s))return l;
	return NULL;
}
