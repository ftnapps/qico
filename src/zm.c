 ...
 * $Id: zm.c,v 1.8 2001/03/10 19:50:19 lev Exp $
 **********************************************************/
/*
...
void zsbhdr(int len, int type, char *hdr)
{
	...
	DEBUG(('Z',1,"zsbhdr: %s", frametypes[type+FTOFFSET]));
	BUFCLEAR();
	if (type == ZDATA)
		for (n = Znulls; --n >=0; )
			BUFCHAR(0);
...
int zgethdr(char *hdr)
{
  ...
  fifi:
	...
	DEBUG(('Z',1,"zgethdr: %s", frametypes[c+FTOFFSET]));
